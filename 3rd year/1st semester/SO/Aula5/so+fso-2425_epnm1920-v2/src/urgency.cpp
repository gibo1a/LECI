...
int doctor_iteration(int id) // return value can be used to request termination
{
   ...
   while(true){
      ...
      mutex_lock(&hd->all_patients[patient].mtx_p);

      hd->all_patients[patient].done = 1;

      cond_broadcast(&hd->all_patients[patient].cond_p_done);

      mutex_unlock(&hd->all_patients[patient].mtx_p);
   }
...
void patient_goto_urgency(int id)
{
   ...
   mutex_init(&hd->all_patients[id].mtx_p,NULL);
   cond_init(&hd->all_patients[id].cond_p_done,NULL);
...
void patient_wait_end_of_consultation(int id)
{
   ...
   while(!hd->all_patients[id].done){
      cond_wait(&hd->all_patients[id].cond_p_done,&hd->all_patients[id].mtx_p);
   }
...
int main(int argc, char *argv[])
{
   ...
   pthread_t p_threads[npatients];
   int patient_id[npatients];
   for(int i = 0; i < npatients; i++)
   {
      patient_id[i] = i;
      thread_create(&p_threads[i],NULL,patient_thread,patient_id+i);
      /*printf("\n");
      random_wait(); // random wait for patience creation
      patient_life(i);*/
   }
   //printf("%s : %d\n","aqui main",hd->triage_queue.cnt);

   pthread_t n_threads[nnurses];
   int nurse_id[nnurses];
   for(int i = 0; i < nnurses; i++)
   {
      nurse_id[i] = i;
      thread_create(&n_threads[i],NULL,nurse_thread,nurse_id+i);
   }

   pthread_t d_threads[ndoctors];
   int doctor_id[ndoctors];
   for(int i = 0; i < ndoctors; i++)
   {
      doctor_id[i] = i;
      thread_create(&d_threads[i],NULL,doctor_thread,doctor_id+i);
   }

   for(int i = 0; i < npatients; i++)
   {
      thread_join(p_threads[i],NULL);
   }
   ...
   for(int i = 0; i < nnurses; i++)
   {
      thread_join(n_threads[i],NULL);
   }

   for(int i = 0; i < ndoctors; i++)
   {
      thread_join(d_threads[i],NULL);
   }
